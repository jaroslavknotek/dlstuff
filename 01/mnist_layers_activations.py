...
activation = act_dict.get(args.activation,None)

input_layers = [tf.keras.layers.InputLayer((MNIST.H, MNIST.W, MNIST.C)),
    tf.keras.layers.Flatten()]

# xTODO: Add `args.layers` number of hidden layers with size `args.hidden_layer`,
# using activation from `args.activation`, allowing "none", "relu", "tanh", "sigmoid".

hidden_layers = [tf.keras.layers.Dense(args.hidden_layer, activation=activation, name="hidden_{}".format(i)) for i in range(args.layers)]
output_layers = [tf.keras.layers.Dense(MNIST.LABELS, activation=tf.nn.softmax),]
